...
{
  /**
   * \brief A type exposing a constexpr initialized array of char buffer
   */
  struct regular_grammar_strings_array
  {
     ...
     * merged together inside an integral sequence
     */
...
# the user of the grammar and are not usefull to create the parser.  Moreover, #
# documentation lines can be inserted everywhere in the grammar definition but #
# cannot be interlaced with other grammar entity like groups or symbols.       #
      )grammar",
      R"grammar(
...
# assignation. By the way, it is impossible to use the special meaning of '.'  #
...
# - The brace pair is also available in group definition. It may be usefull    #
