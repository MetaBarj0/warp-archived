...
namespace warp::spark
{
   ...
   * grammars, or (limited) regular expressions. We start with deterministic
   * finite automata as underlying structure to represent such grammars. Other
   * representations will be discussed for other grammar types. This
   * specialization works with a value or an array type containing a regular
   * grammar definition having a form of a string. It is more friendly for the
   * end-user to declare regular grammar as string rather than using a rough
   * integral sequence.
   *
